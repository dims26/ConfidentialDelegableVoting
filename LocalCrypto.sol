...
library Secp256k1_crypto {
    ...
    function validateSignature(bytes32 message, uint[2] calldata rs, uint[2] calldata Q) internal pure returns (bool) {
        ...
        uint sInv = ECCMath_crypto.invmod(rs[1], n);
        ...
        uint Px = ECCMath_crypto.invmod(P[2], p); // need Px/Pz^2
    ...
    function decompress(uint8 yBit, uint x) internal pure returns (uint[2] memory P) {
        ...
        uint y_ = ECCMath_crypto.expmod(y2, (p + 1) / 4, p);
        uint cmp = yBit ^ y_ & 1;
        P[0] = x;
        P[1] = (cmp == 0) ? y_ : p - y_;
    }
    ...
    function _mul(uint d, uint[2] memory P) internal pure returns (uint[3] memory Q) {
        ...
        INV[7] = ECCMath_crypto.invmod(INV[6], p);             // a7inv
        ...
        for(uint k = 0; k < 7; k++) {
            ECCMath_crypto.toZ1(PREC[k + 1], INV[k + 9], mulmod(INV[k + 9], INV[k + 9], p), p);
        }
...
contract LocalCrypto {
  ...
  function commitToVote(uint[4] calldata params, uint[2] calldata xG, uint[2] calldata yG, uint[2] calldata y,
    uint[2] calldata a1, uint[2] calldata b1, uint[2] calldata a2, uint[2] calldata b2) public view returns (bytes32) {
  ...
  function createZKP(uint x, uint v, uint[2] calldata xG) public returns (uint[4] memory res) {
      ...
      // Get g^{v}
      uint[3] memory vG = Secp256k1_crypto._mul(v, _G);

      // Convert to Affine Co-ordinates
      ECCMath_crypto.toZ1(vG, pp);
  ...
  function verifyZKP(uint[2] memory xG, uint r, uint[3] calldata vG) public view returns (bool){
      uint[2] memory _G;
      _G[0] = Gx;
      _G[1] = Gy;

      // Check both keys are on the curve.
      if(!Secp256k1_crypto.isPubKey(xG) || !Secp256k1_crypto.isPubKey(vG)) {
      ...
      // Get g^{r}, and g^{xc}
      uint[3] memory rG = Secp256k1_crypto._mul(r, _G);
      uint[3] memory xcG = Secp256k1_crypto._mul(c, xG);

      // Add both points together
      uint[3] memory rGxcG = Secp256k1_crypto._add(rG,xcG);

      // Convert to Affine Co-ordinates
      ECCMath_crypto.toZ1(rGxcG, pp);
  ...
  function create1outof2ZKPNoVote(uint[2] calldata xG, uint[2] calldata  yG, uint w, uint r2, uint d2, uint x) public view returns (uint[10] memory res, uint[4] memory res2) {
      ...
      // y = h^{x} * g
      uint[3] memory temp1 = Secp256k1_crypto._mul(x,yG);
      ECCMath_crypto.toZ1(temp1, pp);
      ...
      // a1 = g^{w}
      temp1 = Secp256k1_crypto._mul(w,G);
      ECCMath_crypto.toZ1(temp1, pp);
      ...
      // b1 = h^{w} (where h = g^{y})
      temp1 = Secp256k1_crypto._mul(w, yG);
      ECCMath_crypto.toZ1(temp1, pp);
      ...
      // a2 = g^{r2} * g^{x*d2}
      temp1 = Secp256k1_crypto._mul(r2,G);
      temp1 = Secp256k1_crypto._add(temp1, Secp256k1_crypto._mul(d2,xG));
      ECCMath_crypto.toZ1(temp1, pp);
      ...
      temp1 = Secp256k1_crypto._add(Secp256k1_crypto._mul(d2,temp_affine2), Secp256k1_crypto._mul(d2,temp_affine1));

      // Now... it is h^{r2} + temp2..
      temp1 = Secp256k1_crypto._add(Secp256k1_crypto._mul(r2,yG),temp1);
  ...
  function create1outof2ZKPYesVote(uint[2] calldata xG, uint[2] calldata yG, uint w, uint r1, uint d1, uint x) public view returns (uint[10] memory res, uint[4] memory res2) {
      ...
      uint[3] memory temp1 = Secp256k1_crypto._mul(x,yG);
      Secp256k1_crypto._addMixedM(temp1,G);
      ECCMath_crypto.toZ1(temp1, pp);
      res[0] = temp1[0];
      res[1] = temp1[1];

      // a1 = g^{r1} * g^{x*d1}
      temp1 = Secp256k1_crypto._mul(r1,G);
      temp1 = Secp256k1_crypto._add(temp1, Secp256k1_crypto._mul(d1,xG));
      ECCMath_crypto.toZ1(temp1, pp);
      res[2] = temp1[0];
      res[3] = temp1[1];

      // b1 = h^{r1} * y^{d1} (temp = affine 'y')
      temp1 = Secp256k1_crypto._mul(r1,yG);
      ...
      temp1= Secp256k1_crypto._add(temp1, Secp256k1_crypto._mul(d1, temp));
      ECCMath_crypto.toZ1(temp1, pp);
      res[4] = temp1[0];
      res[5] = temp1[1];
      ...
      //id seems to be sender address, prolly makes it easier than above
      bytes32 b_c = sha256(abi.encodePacked(msg.sender, xG, res));
      uint c = uint(b_c);
  ...
  function verify1outof2ZKP(uint[4] calldata params, uint[2] calldata xG, uint[2] calldata yG, uint[2] calldata y, uint[2] calldata a1, uint[2] calldata b1, uint[2] calldata a2, uint[2] calldata b2) public view returns (bool) {
      uint[2] memory temp1;
      uint[3] memory temp2;
      uint[3] memory temp3;

      // Make sure we are only dealing with valid public keys!
      if(!Secp256k1_crypto.isPubKey(xG) || !Secp256k1_crypto.isPubKey(yG) || !Secp256k1_crypto.isPubKey(y) || !Secp256k1_crypto.isPubKey(a1) ||
         !Secp256k1_crypto.isPubKey(b1) || !Secp256k1_crypto.isPubKey(a2) || !Secp256k1_crypto.isPubKey(b2)) {
         return false;
      }
      ...
      // a1 =? g^{r1} * x^{d1}
      temp2 = Secp256k1_crypto._mul(params[2], G);
      temp3 = Secp256k1_crypto._add(temp2, Secp256k1_crypto._mul(params[0], xG));
      ECCMath_crypto.toZ1(temp3, pp);
      ...
      //b1 =? h^{r1} * y^{d1} (temp = affine 'y')
      temp2 = Secp256k1_crypto._mul(params[2],yG);
      temp3 = Secp256k1_crypto._add(temp2, Secp256k1_crypto._mul(params[0], y));
      ECCMath_crypto.toZ1(temp3, pp);
      ...
      //a2 =? g^{r2} * x^{d2}
      temp2 = Secp256k1_crypto._mul(params[3],G);
      temp3 = Secp256k1_crypto._add(temp2, Secp256k1_crypto._mul(params[1], xG));
      ECCMath_crypto.toZ1(temp3, pp);
      ...
      // y-g
      temp2 = Secp256k1_crypto._addMixed(temp3,temp1);

      // Return to affine co-ordinates
      ECCMath_crypto.toZ1(temp2, pp);
      temp1[0] = temp2[0];
      temp1[1] = temp2[1];

      // (y-g)^{d2}
      temp2 = Secp256k1_crypto._mul(params[1],temp1);

      // Now... it is h^{r2} + temp2..
      temp3 = Secp256k1_crypto._add(Secp256k1_crypto._mul(params[3],yG),temp2);
  ...
  function createCommitment(uint r, uint b) internal view returns (uint[2] memory c_affine){

        uint[3] memory bG = Secp256k1_crypto._mul(b,G);

        uint[3] memory rY = Secp256k1_crypto._mul(r,Y);

        uint[3] memory c = Secp256k1_crypto._add(bG,rY);

        ECCMath_crypto.toZ1(c, pp);

        c_affine[0] = c[0];
        c_affine[1] = c[1];

        // Sanity check that everything worked as expected.
        if(!Secp256k1_crypto.isPubKey(c_affine)) {
            revert(); //Must be on the curve!
        }
  ...
  function openCommitment(uint[2] calldata c, uint r, uint b) public view returns (bool) {
  ...
  // return t,n.
  function createEqualityProof(uint r1, uint r2, uint r3, uint[2] calldata c1, uint[2] calldata c2) public view returns (uint[2] memory t, uint n) {

        if(!Secp256k1_crypto.isPubKey(c1)) {
            revert(); //Must be on the curve!
        }

        if(!Secp256k1_crypto.isPubKey(c2)) {
            revert(); //Must be on the curve!
        }

        uint[3] memory r3Y = Secp256k1_crypto._mul(r3,Y);
        ECCMath_crypto.toZ1(r3Y, pp);
  ...
  function computeFirstHalfEquality(uint[2] calldata c1, uint[2] calldata c2, uint h, uint[2] calldata t) public pure returns (uint[2] memory left){
        ...
        // c1 - c2
        uint[3] memory added_commitments_jacob = Secp256k1_crypto._addMixed(negative_c2,c1);

        // convert to affine points
        ECCMath_crypto.toZ1(added_commitments_jacob,pp);
        ...
        // h*(c1-c2) + t
        uint[3] memory left_jacob = Secp256k1_crypto._addMixed(Secp256k1_crypto._mul(h,added_commitments), t);
        ECCMath_crypto.toZ1(left_jacob,pp);
        left[0] = left_jacob[0];
        left[1] = left_jacob[1];
  ...
  // 2. Does nY == h*(c1-c2) + t
  function verifyEqualityProof(uint n,  uint[2] calldata c1, uint[2] calldata c2, uint[2] calldata t) public view returns (bool) {
        if(!Secp256k1_crypto.isPubKey(c1)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(c2)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(t)) { revert(); }
        ...
        // n * Y
        uint[3] memory right = Secp256k1_crypto._mul(n,Y);

        ECCMath_crypto.toZ1(right, pp);
  ...
  function createInequalityProof(uint b1, uint b2, uint r1, uint r2, uint r3, uint r4, uint[2] calldata c1, uint[2] calldata c2) public view returns (uint[2] memory t1, uint[2] memory t2, uint n1, uint n2) {

        if(!Secp256k1_crypto.isPubKey(c1)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(c2)) { revert(); }

        // r3 * G
        uint[3] memory temp = Secp256k1_crypto._mul(r3,G);
        ECCMath_crypto.toZ1(temp, pp);
        t1[0] = temp[0];
        t1[1] = temp[1];

        // r4 * Y
        temp = Secp256k1_crypto._mul(r4,Y);
        ECCMath_crypto.toZ1(temp, pp);
        t2[0] = temp[0];
        t2[1] = temp[1];
  ...
  function computeSecondHalfInequality(uint[2] calldata c1, uint[2] calldata c2, uint[2] calldata t2, uint h) public pure returns (uint[3] memory right) {
        uint[3] memory negative_c2;
        // Negate the 'y' co-ordinate of C2
        negative_c2[0] = c2[0];
        negative_c2[1] = pp - c2[1];
        ...
        // h(c1-c2)
        uint[3] memory h_mul_c1c2 = Secp256k1_crypto._mul(h,added_commitments);

        // right hand side h(c1-c2) + t2
        right = Secp256k1_crypto._addMixed(h_mul_c1c2,t2);
        ECCMath_crypto.toZ1(right,pp);
  ...
  function verifyInequalityProof(uint[2] calldata c1, uint[2] calldata c2, uint[2] calldata t1, uint[2] calldata t2, uint n1, uint n2) public view returns (bool) {
        if(!Secp256k1_crypto.isPubKey(c1)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(c2)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(t1)) { revert(); }
        if(!Secp256k1_crypto.isPubKey(t2)) { revert(); }
        ...
        // n2 * Y
        uint[3] memory n2Y = Secp256k1_crypto._mul(n2,Y);
        ECCMath_crypto.toZ1(n2Y,pp); // convert to affine

        if(n2Y[0] != right[0] && n2Y[1] != right[1]) {

        // h(c1 - c2) + t2 + t1
        uint[3] memory h_c1c2_t2_t1 = Secp256k1_crypto._addMixed(right, t1);
        ECCMath_crypto.toZ1(h_c1c2_t2_t1,pp); // convert to affine
        right[0] = h_c1c2_t2_t1[0];
        right[1] = h_c1c2_t2_t1[1];

        // n1G + n2Y
        uint[3] memory n1Gn2Y = Secp256k1_crypto._add(Secp256k1_crypto._mul(n1, G),n2Y);
        ECCMath_crypto.toZ1(n1Gn2Y,pp); // convert to affine
